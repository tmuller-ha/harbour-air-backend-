...
export default factories.createCoreService('plugin::forms.chartered-flight-request', 
  ({ strapi }: { strapi: Strapi }) => ({
    async getAll(query) {
      ...
      const data = await strapi.entityService?.findMany('plugin::forms.chartered-flight-request',{
        start: start,
        limit: limit
      });
      const totalData = await strapi.entityService?.findMany('plugin::forms.chartered-flight-request');
